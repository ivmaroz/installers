...
if [ ! -f /usr/local/bin/vmagent ] || [ "$(shasum -a256 "${VM_SOURCE_DIR}/vmagent-prod" | awk '{ print $1 }')" != "$(shasum -a256 /usr/local/bin/vmagent | awk '{ print $1 }')" ]; then
  sudo cp -v "${VM_SOURCE_DIR}/vmagent-prod" /usr/local/bin/vmagent
  UPDATED=1
fi
...
if [ ! -f /etc/victoriametrics/vmagent.yml ]; then
  sudo cp -v "${SCRIPT_DIR}/victoriametrics/etc/victoriametrics/vmagent.yml" /etc/victoriametrics/vmagent.yml
fi
